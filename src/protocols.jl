...
function writeMessageBegin(p::TBinaryProtocol, name::AbstractString, mtype::Int32, seqid::Integer)
    ...
    if p.strict_write
        writeI32(p, reinterpret(Int32, UInt32(BINARY_VERSION_1 | mtype)))
        writeString(p, name)
        writeI32(p, seqid)
    else
        writeString(p, name)
...
write(p::TBinaryProtocol, s::ASCIIString)       = (writeI32(p, length(s.data)); write(p, s.data); nothing)
write(p::TBinaryProtocol, s::UTF8String)        = (writeI32(p, length(s.data)); write(p, s.data); nothing)
...
writeVarint{T <: Integer}(p::TCompactProtocol, i::T) = (_write_uleb(rawio(p.t), i); nothing)
...
write(p::TCompactProtocol, s::ASCIIString)      = (writeSize(p, length(s.data)); write(p, s.data); nothing)
write(p::TCompactProtocol, s::UTF8String)       = (writeSize(p, length(s.data)); write(p, s.data); nothing)
